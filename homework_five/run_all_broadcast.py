...
def main():
    parser = argparse.ArgumentParser(description='Run all Broadcast benchmarks and generate plots.')
    parser.add_argument('--out-dir', default='broadcast_bench')
    parser.add_argument('--iters', type=int, default=3)
    parser.add_argument('--world-sizes', nargs='+', type=int, default=[2, 4, 8, 16])
    parser.add_argument('--message-sizes-bytes', nargs='+', type=int, default=[1024, 4096, 16384, 65536, 1048576, 4194304])
    parser.add_argument('--fixed-msg-bytes', type=int, default=1048576)
    parser.add_argument('--fixed-world-size', type=int, default=8)
    parser.add_argument('--root', type=int, default=0, help='Broadcast root rank')
    ...
    results_file = out_dir / 'broadcast_results.json'
    results_file.write_text(json.dumps(results, indent=2))
    print(f'Wrote {results_file}')

    run([
        sys.executable,
        'generate_plots_broadcast.py',
