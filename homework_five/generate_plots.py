...
def plot_vs_message_size(results, output_dir: Path, fixed_world_size: int, algorithms):
    ...
    plt.title(f"AllGather on Gloo vs message size (world size = {fixed_world_size})")
    plt.grid(True, which="both", alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "allgather_vs_message_size.png", dpi=180)
    plt.close()
...
def plot_vs_world_size(results, output_dir: Path, fixed_msg_bytes: int, algorithms):
    ...
    plt.ylabel("Completion time (ms)")
    plt.title(f"AllGather on Gloo vs number of ranks (message size = {fixed_msg_bytes // 1024} KB per rank)")
    plt.grid(True, alpha=0.3)
    plt.legend()
    plt.tight_layout()
    plt.savefig(output_dir / "allgather_vs_world_size.png", dpi=180)
    plt.close()


def main():
    parser = argparse.ArgumentParser(description="Generate AllGather benchmark plots from JSON or JSONL results.")
    parser.add_argument("--input", required=True, help="Path to a JSON or JSONL results file")
    parser.add_argument("--output-dir", default="allgather_bench", help="Directory to write the PNG plots")
    ...
    parser.add_argument(
        "--algorithms",
        nargs="+",
        default=["ring", "recursive_doubling", "swing"],
    ...
    combined_path = output_dir / "allgather_results.json"
    ...
    print(f"Wrote {output_dir / 'allgather_vs_message_size.png'}")
    print(f"Wrote {output_dir / 'allgather_vs_world_size.png'}")
